...
from metadata_schema import (ProductSchema, Product, Domain, Horizon, Issue,
                             Request, Variable, User, db)
from datetime import datetime, timedelta, time, timezone
import json
import sys
import re

csv.field_size_limit(10000)#sys.maxsize)
...
def db_build():
    with open('DB_Backup/Tbl_prod_backup.csv') as csvfile:
        ...
        for row in reader:
            prod = Product(product_id=int(row['prodkey']), key=row['prod'],
                           name=row['product_name'],
                           start_date=datetime.strptime(row['startdate'], '%Y-%m-%d %H:%M:%S'),
                           end_date=datetime.strptime(row['enddate'], '%Y-%m-%d %H:%M:%S'))
            prod.horizons = []
            prod.issues = []
            prod.variables = []
            prod.requests = []
    ...
    with open('DB_Backup/Tbl_horizon_backup.csv') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            horizon = Horizon(horizon_id=int(row['horizonkey']),
                              horizon=int(row['avail_horizon']))
            m[row['product_id']].horizons.append(horizon)
            db.session.add(horizon)
    with open('DB_Backup/Tbl_issues_backup.csv') as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            issue = Issue(issue_id=int(row['issuekey']),
                          issue=time(hour=int(row['avail_issues'])))
    ...
    with open('DB_Backup/Tbl_request_backup.csv') as csvfile:
        ...
        for row in reader:
            ...
            if(reqid > 137):
                req = Request(request_id=row['requestkey'],
                              request_name=row['request_id'],
                              request_valid=(row['valid'] == 't'),
